...
def isblocked(x, y, rocks, sands):
    return (x, y) in rocks or (x, y) in sands

def simulate_step(sand_unit, rocks, sands):
    x, y = sand_unit
    if not isblocked(x, y+1, rocks, sands):
        return (x, y+1), False
    elif not isblocked(x-1, y+1, rocks, sands):
...
def simulate(sand, rocks, sands, max_depth):
    blocked = False
    while not blocked:
        sand, blocked = simulate_step(sand, rocks, sands)
        if blocked:
            sands.append(sand)
            return sands, False
        elif sand[1] >= max_depth:
